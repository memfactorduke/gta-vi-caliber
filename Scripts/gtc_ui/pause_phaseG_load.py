"""
pause_phaseG_load.py — PHASE D slice 4: the LOAD tab content panel (save slots).

Builds GTC_Panel_Load (CanvasPanel container, same right region, HIDDEN by default)
with a rounded frame, "LOAD GAME" header, and a save-slot list: slot name (left,
bright) + meta timestamp/completion (right, dim). First slot highlighted cyan
(selected). Static demo slots for now. Same VerticalBox auto-stack pattern.
Idempotent. Compiles + saves.

Run:  py "/Users/ziwenxu/Desktop/Code/GTA6-unreal/GT-caliber_5.8/Scripts/gtc_ui/pause_phaseG_load.py"
"""
...
OUT = "/private/tmp/claude-501/-Users-ziwenxu-Desktop-Code-GTA6-unreal-GT-caliber-5-8/d8d4bf46-eade-49bd-9282-7189b8262c91/scratchpad/pause_phaseG.json"
...
TEXT_FAINT = (0.40, 0.45, 0.49, 1.0)

ROWS = [
    ("AUTOSAVE", "12 MIN AGO    42%"),
    ("SLOT 01", "DAY 03  13:42    42%"),
    ("SLOT 02", "DAY 01  09:15    18%"),
    ("SLOT 03", "DAY 02  21:30    27%"),
    ("SLOT 04", "- EMPTY -"),
    ("SLOT 05", "- EMPTY -"),
]
...
try:
    ...
    panel = ensure(mwc, "GTC_Panel_Load", unreal.CanvasPanel)
    canvas_anchor(panel, 0.500, 0.160, 0.955, 0.860, 4)
    panel.set_visibility(unreal.SlateVisibility.HIDDEN)

    frame = ensure(panel, "GTC_LoadFrame", unreal.Image)
    ...
    hdr = ensure(panel, "GTC_LoadHeader", unreal.TextBlock)
    hdr.set_editor_property("font", font(22))
    hdr.set_editor_property("text", "LOAD GAME")
    ...
    canvas_anchor(hdr, 0.05, 0.045, 0.7, 0.13, 2)

    labels = ensure(panel, "GTC_LoadLabels", unreal.VerticalBox)
    canvas_anchor(labels, 0.06, 0.17, 0.45, 0.95, 2)
    values = ensure(panel, "GTC_LoadValues", unreal.VerticalBox)
    canvas_anchor(values, 0.42, 0.17, 0.94, 0.95, 2)
    ...
    def add_row(vbox, name, txt, color, justify):
        ...
        sl = t.slot
        try:
            sl.set_padding(unreal.Margin(0.0, 11.0, 0.0, 11.0))
    ...
    for i, (lbl, val) in enumerate(ROWS):
        empty = val.strip().startswith("-")
        add_row(labels, "GTC_LoadL_%02d" % i, lbl, (TEXT_FAINT if empty else TEXT_BRIGHT), unreal.TextJustify.LEFT)
        add_row(values, "GTC_LoadV_%02d" % i, val, TEXT_FAINT if empty else TEXT_DIM, unreal.TextJustify.RIGHT)

    # highlight first slot (selected)
    try:
...
unreal.log("GTC_PHASEG_DONE " + json.dumps(res))
