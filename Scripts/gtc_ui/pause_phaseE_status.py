"""
pause_phaseE_status.py — PHASE D slice 2: the STATUS tab content panel.

Builds GTC_Panel_Status (a CanvasPanel container) anchored to the SAME right region
as the map, HIDDEN by default (so it doesn't overlap the map until tab-switching is
wired). Inside: rounded glass frame, "STATUS" header, and a two-column stat list
(labels left / values right) built from VerticalBoxes so rows auto-stack and stay
aligned at any resolution. Static demo values for now (live binding needs graph/C++).
Idempotent. Compiles + saves.

Run:  py "/Users/ziwenxu/Desktop/Code/GTA6-unreal/GT-caliber_5.8/Scripts/gtc_ui/pause_phaseE_status.py"
"""
...
OUT = "/private/tmp/claude-501/-Users-ziwenxu-Desktop-Code-GTA6-unreal-GT-caliber-5-8/d8d4bf46-eade-49bd-9282-7189b8262c91/scratchpad/pause_phaseE.json"
...
ROWS = [
    ...
    ("CASH", "$48,250"),
    ("DISTANCE", "12.4 MI"),
    ("TIME PLAYED", "00:42:18"),
    ("AREA", "OCEAN DR / VICEPORT"),
]
...
try:
    ...
    # ---- container ----
    panel = ensure(mwc, "GTC_Panel_Status", unreal.CanvasPanel)
    canvas_anchor(panel, 0.500, 0.160, 0.955, 0.860, 4)
    panel.set_visibility(unreal.SlateVisibility.HIDDEN)  # hidden until tab-switch wired

    # ---- frame ----
    frame = ensure(panel, "GTC_StatusFrame", unreal.Image)
    ...
    # ---- header ----
    hdr = ensure(panel, "GTC_StatusHeader", unreal.TextBlock)
    hdr.set_editor_property("font", font(22))
    hdr.set_editor_property("text", "STATUS")
    ...
    # ---- two columns ----
    labels = ensure(panel, "GTC_StatusLabels", unreal.VerticalBox)
    canvas_anchor(labels, 0.06, 0.17, 0.52, 0.93, 2)
    values = ensure(panel, "GTC_StatusValues", unreal.VerticalBox)
    canvas_anchor(values, 0.50, 0.17, 0.94, 0.93, 2)

    def add_row(vbox, name, txt, color, justify):
        t = ensure(vbox, name, unreal.TextBlock)
        t.set_editor_property("font", font(17))
        ...
        sl = t.slot  # VerticalBoxSlot
        try:
            sl.set_padding(unreal.Margin(0.0, 9.0, 0.0, 9.0))
    ...
    for i, (lbl, val) in enumerate(ROWS):
        add_row(labels, "GTC_StatL_%02d" % i, lbl, TEXT_DIM, unreal.TextJustify.LEFT)
        add_row(values, "GTC_StatV_%02d" % i, val, TEXT_BRIGHT, unreal.TextJustify.RIGHT)
...
unreal.log("GTC_PHASEE_DONE " + json.dumps(res))
