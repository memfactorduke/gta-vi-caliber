#!/usr/bin/env bash
# Headless verifier for the Wings & Waves fast-travel pure-cores.
#
# Each *_verify.cpp in this folder #includes the REAL shipping core .cpp from
# Source/GTC_UE5/World/FastTravel and re-asserts the same invariants the GTC.* automation
...
# (UnrealEditor-Cmd … Automation RunTests GTC.World.FastTravel) is intentionally NOT invoked:
...
if [ "$fail" -ne 0 ]; then
	echo "=== fast-travel checks FAILED ==="
	exit 1
fi
echo "=== all fast-travel checks PASSED ==="
