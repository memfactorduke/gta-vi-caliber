#!/usr/bin/env bash
# Headless verifier for the Wings & Waves aircraft/vehicle kinematic pure-cores.
#
# Each *_verify.cpp in this folder #includes the REAL shipping core .cpp from
# Source/GTC_UE5/Vehicles and re-asserts the same invariants the GTC.* automation
...
if [ "$fail" -ne 0 ]; then
	echo "=== aircraft checks FAILED ==="
	exit 1
fi
echo "=== all aircraft checks PASSED ==="
