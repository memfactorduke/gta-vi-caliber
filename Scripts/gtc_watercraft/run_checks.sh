#!/usr/bin/env bash
# Headless verifier for the Wings & Waves watercraft kinematic pure-cores.
#
# Each *_verify.cpp in this folder #includes the REAL shipping core .cpp from
# Source/GTC_UE5/Vehicles/Watercraft and re-asserts the same invariants the GTC.* automation
...
# (UnrealEditor-Cmd … Automation RunTests GTC.Vehicles) is intentionally NOT invoked:
...
if [ "$fail" -ne 0 ]; then
	echo "=== watercraft checks FAILED ==="
	exit 1
fi
echo "=== all watercraft checks PASSED ==="
