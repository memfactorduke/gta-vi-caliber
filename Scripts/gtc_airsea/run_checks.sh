#!/usr/bin/env bash
# Headless verifier for the Wings & Waves air/sea police escalation + naval pursuit pure-cores.
#
# Each *_verify.cpp in this folder #includes the REAL shipping core .cpp from
# Source/GTC_UE5/AI and re-asserts the same invariants the GTC.* automation
...
# (UnrealEditor-Cmd … Automation RunTests GTC.AI) is intentionally NOT invoked:
...
if [ "$fail" -ne 0 ]; then
	echo "=== air/sea checks FAILED ==="
	exit 1
fi
echo "=== all air/sea checks PASSED ==="
