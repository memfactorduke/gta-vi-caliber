	...
	if (!Sun)
	{
		...
		if (Sun)
		{
			...
			if (UDirectionalLightComponent* C = Cast<UDirectionalLightComponent>(Sun->GetLightComponent()))
			{
				...
				// Win the single directional slot used for forward shading,
				// translucency, single-layer water and volumetric fog. Without an
				// explicit priority the engine ties at 0 and falls back to overall
				// brightness, which lets the night-time moon fill (EnsureMoon) steal
				// the slot from the real sun. A high priority pins it to the sun.
				C->SetForwardShadingPriority(SunForwardShadingPriority);
			}
		}
	}
	if (!SkyLight)
	...
	if (!Sun)
	{
		...
		if (L)
		{
			if (UDirectionalLightComponent* C = Cast<UDirectionalLightComponent>(L->GetLightComponent()))
			{
				...
				C->SetForwardShadingPriority(SunForwardShadingPriority); // own the forward-shading slot
...
void AGTCWeatherController::EnsureMoon()
{
	...
	if (Moon)
	{
		// An adopted moon must stay out of the forward-shading contest so it never
		// outranks the sun on brightness at night (see the sun's priority above).
		if (UDirectionalLightComponent* C = Cast<UDirectionalLightComponent>(Moon->GetLightComponent()))
		{
			C->SetForwardShadingPriority(MoonForwardShadingPriority);
		}
	}
	if (!Moon)
	{
		...
		if (L)
		{
			if (UDirectionalLightComponent* C = Cast<UDirectionalLightComponent>(L->GetLightComponent()))
			{
				...
				C->SetForwardShadingPriority(MoonForwardShadingPriority); // never contend with the sun
				C->SetIntensity(0.0f);
