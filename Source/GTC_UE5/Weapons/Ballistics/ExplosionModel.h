 ...
 * of the Godot `ExplosionModel` (RefCounted) at
 ...
 * Unit-tested headless via the parity oracle (Tests/ExplosionModelTest.cpp,
 ...
 *
 * NOTE: no Godot->UE Z-up axis remap — the model stays in the Godot frame, and
 * the upward knockback bias is along Godot's +Y (FVector::UpVector here is the
 * literal Godot `Vector3.UP` = (0,1,0)). Axis remap is a DEFERRED Wave-3 concern.
 *
...
struct GTC_UE5_API FExplosionModel
{
     ...
     * only on real hits.
     *
     * (Godot's defensive `not (entry is Vector3)` skip is unrepresentable with a
