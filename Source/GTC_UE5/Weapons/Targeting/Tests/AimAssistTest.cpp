...
bool FAimAssistTest::RunTest(const FString& Parameters)
{
    ...
    const FVector FwdDir(1.0, 0.0, 0.0);
    ...
    TestTrue(TEXT("target dead ahead is ~0 off-axis"),
        FMath::IsNearlyEqual(FAimAssist::AngleToTargetRad(Origin, FwdDir, FVector(10, 0, 0)), 0.0, GtcTest::Eps));
    TestTrue(TEXT("target at 90 degrees is ~pi/2"),
        FMath::IsNearlyEqual(FAimAssist::AngleToTargetRad(Origin, FwdDir, FVector(0, 10, 0)), PI / 2.0, GtcTest::Eps));
    TestTrue(TEXT("target behind is ~pi"),
        FMath::IsNearlyEqual(FAimAssist::AngleToTargetRad(Origin, FwdDir, FVector(-10, 0, 0)), PI, GtcTest::Eps));
    TestEqual(TEXT("target on the origin is inert"),
        FAimAssist::AngleToTargetRad(Origin, FwdDir, Origin), 0.0);

    // --- Score gating ---
    TestTrue(TEXT("a centred, in-range, hostile target scores > 0"),
        FAimAssist::Score(Origin, FwdDir, MakeCandidate(FVector(10, 0, 0)), Cone, Range) > 0.0);
    TestEqual(TEXT("a non-targetable candidate scores 0"),
        FAimAssist::Score(Origin, FwdDir, MakeCandidate(FVector(10, 0, 0), 1.0, /*bTargetable*/ false), Cone, Range), 0.0);
    TestEqual(TEXT("a zero-priority candidate scores 0"),
        FAimAssist::Score(Origin, FwdDir, MakeCandidate(FVector(10, 0, 0), 0.0), Cone, Range), 0.0);
    TestEqual(TEXT("a candidate beyond range scores 0"),
        FAimAssist::Score(Origin, FwdDir, MakeCandidate(FVector(Range + 5.0, 0, 0)), Cone, Range), 0.0);
    ...
    TestEqual(TEXT("a candidate outside the cone scores 0"),
        FAimAssist::Score(Origin, FwdDir, MakeCandidate(FVector(10, 10, 0)), Cone, Range), 0.0);

    // --- Score ordering ---
    // Same range, more centred scores higher.
    const double Centred = FAimAssist::Score(Origin, FwdDir, MakeCandidate(FVector(10, 0, 0)), Cone, Range);
    const double OffAxis = FAimAssist::Score(Origin, FwdDir, MakeCandidate(FVector(10, 1.0, 0)), Cone, Range);
    TestTrue(TEXT("more centred scores higher than off-axis"), Centred > OffAxis);
    // Same bearing, closer scores higher.
    const double Near = FAimAssist::Score(Origin, FwdDir, MakeCandidate(FVector(10, 0, 0)), Cone, Range);
    const double Far = FAimAssist::Score(Origin, FwdDir, MakeCandidate(FVector(40, 0, 0)), Cone, Range);
    TestTrue(TEXT("closer scores higher than farther"), Near > Far);
    // Higher priority scores higher, all else equal.
    const double LowPri = FAimAssist::Score(Origin, FwdDir, MakeCandidate(FVector(10, 0, 0), 1.0), Cone, Range);
    const double HiPri = FAimAssist::Score(Origin, FwdDir, MakeCandidate(FVector(10, 0, 0), 2.0), Cone, Range);
    ...
    // --- SelectBestTarget ---
    {
        ...
        TestEqual(TEXT("selects the centred, closer target"),
            FAimAssist::SelectBestTarget(Origin, FwdDir, Cands, Cone, Range), 1);
    }
    {
        ...
        TestEqual(TEXT("returns INDEX_NONE when nothing is acquirable"),
            FAimAssist::SelectBestTarget(Origin, FwdDir, None, Cone, Range), (int32)INDEX_NONE);
        TArray<FAimCandidate> Empty;
        TestEqual(TEXT("empty candidate list -> INDEX_NONE"),
            FAimAssist::SelectBestTarget(Origin, FwdDir, Empty, Cone, Range), (int32)INDEX_NONE);
    }
    {
    ...
    // --- ApplyAdhesion ---
    {
        ...
        const double Before = FAimAssist::AngleToTargetRad(Origin, FwdDir, Target);

        // No strength leaves the aim untouched.
        const FVector Zero = FAimAssist::ApplyAdhesion(FwdDir, Origin, Target, 0.0);
        ...
        const FVector Half = FAimAssist::ApplyAdhesion(FwdDir, Origin, Target, 0.5);
        ...
        const FVector Full = FAimAssist::ApplyAdhesion(FwdDir, Origin, Target, 1.0);
        ...
        TestTrue(TEXT("stronger adhesion beats weaker"),
            FAimAssist::AngleToTargetRad(Origin, FAimAssist::ApplyAdhesion(FwdDir, Origin, Target, 0.8), Target) < HalfErr);
