...
/**
 * FWorldSaveGame — the pure-model UE port of the reference `save_game.gd` (class `SaveGame`,
 ...
 * layer: the the reference `SaveGame` envelope is `{version, state}` with VERSION == 1, whereas
 ...
 * the `state` payload, mirroring the reference observable Dictionary key order. No engine USaveGame,
 * no FArchive: serialization is JSON text exactly like the the reference static SaveGame API.
 *
 * The the reference file also exposes write()/read()/has_save() over `user://`; those map to the
 ...
 *
 * Static-style free functions in a namespace (no UObject), exactly like the reference static
 * `SaveGame` API. The owning UObject layer is UWorldSerializerSubsystem.
 */
namespace FWorldSaveGame
{
    /** Save format version. Matches the reference `SaveGame.VERSION`. */
     ...
     * like the reference deserialize(). Silent on malformed input (a normal first-run path).
     */
    GTC_UE5_API TSharedRef<FGtcJsonObject> Deserialize(const FString& Text);

    /** Serialize `State` and write it to `Path`. Returns success (mirrors the reference write()). */
     ...
     * (mirrors the reference read()'s file_exists early-out). Never crashes on a corrupt file.
     */
    GTC_UE5_API TSharedRef<FGtcJsonObject> Read(const FString& Path);
    GTC_UE5_API TSharedRef<FGtcJsonObject> Read();

    /** True when a save file exists at `Path` (mirrors the reference has_save()). */
