...
/**
 * Pure "Busted" resolution — the UE 5.7 port of the reference ArrestModel (RefCounted, PURE).
 ...
 *
 * Static, scene-free, RNG-free — exactly like the the reference static helpers. Plain C++ value
 * type (no UObject); the UArrestSubsystem (the arrest_controller port) owns the grapple
 * timer and drives these helpers from live positions. Headless-testable: reference behavior
 * game/tests/unit/test_arrest_model.gd (15 assertions, mapped 1:1).
 *
 * Parity note: all math is `double`. Constants mirror the the reference const literals exactly.
 * cash_after_bust uses floor (the reference floori) of float(wallet) * (1 - clamp(penalty)),
 * floored at 0; bust_fee is the complement (wallet - kept), floored at 0.
 */
struct GTC_UE5_API FArrestModel
{
    /** Seconds the player must be cornered before the cuffs go on (the reference DEFAULT_GRAPPLE_TIME). */
    static constexpr double DefaultGrappleTime = 1.5;

    /** Fraction of the wallet forfeited on a bust (the reference DEFAULT_CASH_PENALTY). */
     ...
     * range (distance <= catch_distance). Mirrors the reference ArrestModel.cornered.
     */
     ...
     * zero) the instant the player breaks free. Mirrors the reference ArrestModel.tick_grapple:
     ...
     * grapple_time > 0 AND time_held >= grapple_time. Mirrors the reference ArrestModel.is_busted.
     */
     ...
     * Mirrors the reference ArrestModel.cash_after_bust.
     */
     ...
     * Mirrors the reference ArrestModel.bust_fee.
     */
