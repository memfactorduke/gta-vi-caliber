...
public class GTC_UE5Target : TargetRules
{
	public GTC_UE5Target(TargetInfo Target) : base(Target)
	{
		...
		// The installed UE_5.8 UnrealGame was built with these warning levels
		...
		// plugins. A failed full build never regenerates the .target receipt, so
		// the editor prompts "rebuild required" on every launch. Keep them as
		// warnings so the full build completes and the receipt stays in sync.
